...
export default async function getPageProperties(
  ...
  // ── DIAGNOSTIC: author 列排查 ──
  console.log('[AuthorDiag] --- getPageProperties for:', id, 'title:', properties.title)
  console.log('[AuthorDiag] schema keys containing Author:', Object.entries(schema).filter(([k,v])=>v?.name?.toLowerCase?.()?.includes('author')).map(([k,v])=>`${k}: name=${v.name}, type=${v.type}`))
  console.log('[AuthorDiag] raw properties matching Author:', Object.keys(properties).filter(k=>k?.toLowerCase?.()?.includes('author')).map(k=>`${k}=${JSON.stringify(properties[k])}`))
  // ── END DIAGNOSTIC ──
  ...
  console.log('[AuthorDiag] after mapping: properties.author =', JSON.stringify(properties.author))
