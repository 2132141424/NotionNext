...
/**
 * 判断 URL 是否属于 Notion 静态图床（secure.notion-static.com / prod-files-secure）
 * @param {string} url
 * @returns {boolean}
 */
const isNotionStaticHost = url => {
  ...
  try {
    ...
    return (
      ...
      hostname.endsWith('.prod-files-secure.s3.us-west-2.amazonaws.com')
    )
  } catch {
    return false
  }
...
/**
 * 判断 URL 是否为 Notion 官方图片路径（https://www.notion.so/images/）
 * @param {string} url
 * @returns {boolean}
 */
const isNotionImagesUrl = url => {
  if (!url || typeof url !== 'string') {
    return false
  }
  return url.includes('https://www.notion.so/images/')
}
...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  const needConvert =
    !hasConverted &&
    (block.type === 'bookmark' || isNotionStaticHost(ret)) ||
  ...
  // 自定义 CDN 时，把 img.notionusercontent.com 预签名图转成 Notion 永久 /image/ URL 再走 CDN。
  // 预签名 URL 不能简单代理（会过期），必须经由 /image/ 让 Notion 302 到最新签名地址。
  const NOTION_UC_ORIGIN = 'https://img.notionusercontent.com'
  if (
    ret &&
    ret.indexOf(NOTION_UC_ORIGIN) === 0 &&
    BLOG.NOTION_HOST.indexOf('www.notion.so') < 0
  ) {
    try {
      const ucUrl = new URL(ret)
      const encodedS3Path = ucUrl.pathname.replace(/^\/s3\//, '')
      const decodedS3Path = decodeURIComponent(encodedS3Path)
      const s3Url = decodedS3Path.startsWith('https://')
        ? decodedS3Path
        : 'https://prod-files-secure.s3.us-west-2.amazonaws.com/' + decodedS3Path
      ret =
        BLOG.NOTION_HOST +
        '/image/' +
