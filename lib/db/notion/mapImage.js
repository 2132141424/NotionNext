...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  // 自定义CDN时，把 img.notionusercontent.com 预签名图（/s3 路径）也改走 CDN，
  // 由 worker 的 /s3 分流回源；未配置 CDN（仍是 www.notion.so）时保持原样。
  ...
  if (
    ...
    ret = BLOG.NOTION_HOST + ret.slice(NOTION_UC_ORIGIN.length)
  }
